...
自动审核采用分层策略：官方一手信源和可信媒体确认的重大变化可以发布；标题关联不足时保留待复核，不再直接丢弃。高可信新产品可以先进入行业时间线，补齐厂商、官网与定位后再加入完整产品库。

当前版本不需要搜索 API 或模型 API Key。自动规则会拦截传闻、资本噪音、模型跑分和普通更新，并为新产品事件做相似标题去重。
